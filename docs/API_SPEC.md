...
| 연동 참고 | Auth `#20`, Analysis `#35`는 미병합 방향만 참고하고 실제 연동 전 병합본 재확인 |
...
| 기준 가격 유형 | `CURRENT_SALE` | 확인된 현재가 또는 사용자 입력가를 비교 기준으로 선택 |
...
referenceComparablePrice =
  referencePriceType=LIST이면 verified referenceListPrice
  referencePriceType=CURRENT_SALE이면 verified referenceCurrentPrice
...
- 후보 탐색 GET/POST 자체는 Product DB write를 수행하지 않는다.
...
| `matchPercentage` | Integer | O | 0~100 최소 유사도 필터 |
...
- live hydrate 실패 시 허용된 snapshot과 `TEMPORARILY_UNRESOLVED` 상태를 반환한다.
- 표시 가능한 데이터가 없는 항목도 관계를 숨기거나 삭제하지 않고 최소 `productId`,
  `dataStatus`, `savedAt`을 반환한다.
...
- 실제 병합된 principal 타입과 member ID 추출 방식을 확인한다.
- 모든 Product/Recommendation endpoint에 인증 정책을 적용한다.
- Request에 임시 member ID를 추가하지 않는다.
...
`#20`, `#35`가 병합되기 전에는 해당 브랜치 코드를 복제하거나 임시 호환 계층을 만들지 않는다.
