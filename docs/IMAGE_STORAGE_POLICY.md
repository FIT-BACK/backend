...
이 문서는 최종 목표 정책을 정의한다. 2026-07-24 현재 백엔드 구현은 일부 항목이 아직 목표 정책과 다르다.

| 항목 | 현재 구현 | 목표 정책 | 처리 |
|---|---|---|---|
| S3 업로드 방식 | Presigned PUT | Presigned POST | 기능 이슈에서 변경 |
| 발급 API | `POST /api/v1/images/presigned-uploads` | `POST /api/v1/images/upload-requests` | 기능 이슈에서 변경 |
| 업로드 응답 | `requiredHeaders`, `imageUrl` 포함 | `uploadFields` 포함 | 기능 이슈에서 변경 |
| 이미지 용도 | `ANALYSIS_ORIGINAL`, `LOOKBOOK_ORIGINAL`, `LOOKBOOK_MATCHED`, `PROFILE` | `ANALYSIS`, `LOOKBOOK`, `PROFILE` | 기능 이슈에서 변경 |
| 초기 상태 | `PENDING` | `PENDING_UPLOAD` | 신규 마이그레이션으로 변경 |
| Object key | 기존 경로 사용 | `images/{purpose}/{memberId}/{yyyy}/{MM}/{uuid}.{ext}` | 신규 업로드부터 적용 |
| 기존 S3 객체 | 기존 경로에 존재 | 이동하지 않음 | 조회 호환 유지 |
| `retryCount`, `nextRetryAt` | DB에 저장 | 유지 | 기존 구현 유지 |
| multipart 분석 API | 존재 | 전환 기간 유지 | 호환용으로 유지 |
| JSON 분석 API 요청 | 단일 `{ imageId }` | `images[]/role/sourceType` | 별도 기능 이슈에서 전환 |
| `ACTIVE` 마지막 참조 해제 자동 삭제 | 일부 도메인 연동 필요 | 후속 기능 | 완료로 표시하지 않음 |
...
`LOOKBOOK_ORIGINAL`, `LOOKBOOK_MATCHED`처럼 룩북 내부 역할을 `purpose`로 분리하지 않는다. 룩북 내부 역할은 도메인 API의 `images[].role`로 구분한다.
...
(
  ...
  OR status = READY AND uploadedAt < 현재 시각 - 24시간
  OR status = REJECTED AND uploadedAt < 현재 시각 - 24시간
  OR status = DELETE_FAILED AND nextRetryAt <= 현재 시각
)
...
```text
status: PENDING_UPLOAD | READY | ACTIVE | DELETING | DELETE_FAILED | DELETED | REJECTED
visibility: PRIVATE | PUBLIC
purpose: ANALYSIS | LOOKBOOK | PROFILE
```
