...
- [x] 문서 변경만 선별해 커밋하고 원격 브랜치에 푸시한다.
- [x] Issue #204를 연결한 `develop` 대상 draft PR을 생성한다.
- [x] 독립 자체 리뷰를 수행하고 적절한 지적을 반영한다.
- [x] CodeRabbit 피드백을 확인해 적절한 문서 지적을 반영했다. 최신 재검토는 `Review rate limited`이므로 자체 리뷰 결과로 마감한다.
- [x] 최종 변경·검증·미반영 의견을 PR과 사용자에게 보고한다.
