...
| `PUBLIC_BASE_URL` | 외부 HTTPS 검증 활성화 | 경로가 없는 HTTPS origin이다. 현재 값은 `https://d1ra74et9h0ohu.cloudfront.net`이다. 값이 없으면 외부 검증 step은 건너뛴다. |

민감정보는 Repository Variable 또는 GitHub command payload에 넣지 않는다.

## SSM Parameter Store

운영 EC2가 다음 값을 직접 읽는다. 네 값은 모두 `SecureString`으로 생성한다.
...
2. Parameter Store의 DB와 JWT 값을 단일 행 값으로 검증한다.
3. host 단위 `flock`을 획득해 같은 EC2에서 두 배포가 동시에 실행되지 않게 한다.
4. 고유한 `/opt/fitback/releases/<release-id>/.env`에 image와 port 등 비민감 runtime 값만 mode `600`으로 원자적으로 작성하고, DB와 JWT 비밀값은 현재 Compose 프로세스 환경으로 전달한다.
...
- [ ] `/fitback/prod/jwt-secret-key`를 포함한 네 Parameter Store SecureString을 생성했다.
