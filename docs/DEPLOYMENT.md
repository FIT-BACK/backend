...
- 운영 EC2 역할은 현재 Presigned PUT 발급에 필요한 해당 버킷 객체의 `s3:PutObject`만 가진다.
- 업로드 완료 검증과 객체 정리를 구현할 때 필요한 `s3:GetObject`와 `s3:DeleteObject`를 해당 기능과 함께 추가한다.
...
운영 애플리케이션은 시작 시 `schema-prod.sql`의 멱등 DDL을 먼저 실행해 `image` 테이블이
없을 때만 생성한 뒤 Hibernate `ddl-auto=validate`를 수행한다. 이 스크립트는 이미 존재하는
테이블의 컬럼 변경을 수행하지 않는다. 후속 스키마 변경부터는 별도 migration 도구 도입 이슈로
관리한다. 새 빈 DB에서는 선행 도메인 테이블(`member` 등)이 먼저 준비되어 있어야 한다.
