...
`V24__seed_tag_master_taxonomy.sql`을 순서대로 적용하고 Hibernate
...
`STYLE`로 재분류한다.
V24는 확정 태그 43개와 복종 매핑 70개를 멱등하게 구성하고 legacy `베이지톤` 참조를
`베이지`로 수렴시킨다.
...
| Flyway V1~V24 MySQL 적용 | `scripts/ci/test_mysql_migrations.sh` | MySQL 8.4에 모든 migration 적용, 태그 43개·복종 매핑 70개와 기존 주요 제약조건 확인 |
