...
`V6__add_withdrawal_block_and_notification_tables.sql`,
`V7__add_member_delete_cascades.sql`을 순서대로 적용하고 Hibernate
...
| Flyway V1~V7 MySQL DDL | `scripts/ci/test_mysql_migrations.sh` | MySQL 8.4 적용, 기존 `refresh_token` 호환, 이미지 old/new purpose/status, 상품 provider 계약, 회원 알림/탈퇴 cascade 계약 확인 |
