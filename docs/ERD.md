...
현재 운영 DDL은 `src/main/resources/schema-prod.sql`의 멱등 `CREATE TABLE IF NOT EXISTS`로
적용하고 JPA `ddl-auto=validate`로 Entity mapping을 검증한다.
...
| `presigned_expires_at` | `DATETIME(6)` | N | 업로드 URL 만료 시각 |
