...
| --- | --- |
| 기준일 | 2026-07-18 |
| 적용 범위 | Recommendation, Product, 원상품 선택, 상품 찜 |
| 기준 코드 | 현재 `develop`의 `Member`, `AnalysisReport`, `Tag`, `Product`, `ProductTag`, `RecommendedItem` |
| 연동 참고 | Auth `#20`, Analysis `#35`의 방향만 참고하며 병합 전 구현을 선행조건으로 두지 않음 |
...
`USER_ENTERED`는 근거 URL 없이도 허용하지만 확인 시각과 통화는 필수다.
...
FK_RECOMMENDED_ITEM_REFERENCE(reference_selection_id)
  -> reference_product_selection(selection_id) ON DELETE SET NULL ON UPDATE RESTRICT
...
referenceComparablePrice =
  referencePriceType=LIST이면 검증된 referenceListPrice
  referencePriceType=CURRENT_SALE이면 검증된 referenceCurrentPrice
...
- principal의 구체 클래스가 병합되기 전 임시 헤더나 임의 member ID DTO를 추가하지 않는다.
...
- 외부 호출 뒤 input revision 또는 reference selection ID/revision이 달라졌으면 새 세트를
  저장하지 않는다.
- `#20`, `#35`가 미병합인 동안에는 위 FK/소유권 계약만 참고하고 해당 브랜치 코드를 복제하지 않는다.
...
| `user_profiles`, `closet_items`, `refresh_tokens` | `DEFERRED` | Member/Closet/Auth 담당 범위 |
