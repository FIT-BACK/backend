...
Content 계정으로 트렌드별 룩북 샘플을 1개씩 생성했다. 원본은 사용자가 상의 또는 하의 영역을
잘라 올린 것처럼 구성하고, 매칭 이미지는 같은 옷 종류를 다른 모델·자세·배경에서 촬영한 착용
사진으로 구성했다.
...
- 룩북 상세 응답의 원본·매칭 이미지 URL 12개가 모두 HTTP 200으로 열리는 것을 확인했다.
- 각 룩북은 `GET /api/v1/trends/{trendId}/lookbooks?pageSize=3` 첫 페이지에 포함된다.
- 현재는 트렌드별 1개만 준비했다. 최종 목표는 트렌드별 3개이며 각 트렌드에 2개씩 추가가 필요하다.
...
6. 트렌드 1~6에 대해 룩북 ID `7~12`가 각각 조회되는지 확인한다.
...
- 트렌드 1~6의 관련 룩북 첫 페이지에서 룩북 ID 7~12가 각각 조회된다.
- 트렌드 룩북의 원본은 선택한 옷 부위 중심으로, 매칭 이미지는 별도 착용 사진으로 표시된다.
